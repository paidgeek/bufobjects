...
protected byte[] name;protected float speed;protected rpg.inventory.Inventory bag;protected rpg.inventory.Item mainHand;protected double[] buffs;
...
{this.name = BufferObjectBuilder.getStringBytes(name);
  this.speed = speed;this.bag = bag;this.mainHand = mainHand;this.buffs = buffs;}
...
public void reset() {
this.name = new byte[0];this.speed = 0.0f;this.bag = null;this.mainHand = null;this.buffs = new double[BUFFS_LENGTH];
...
newCopy.name = new byte[this.name.length];
    System.arraycopy(this.name, 0, newCopy.name, 0, this.name.length);newCopy.speed = this.speed;if(this.bag != null) {
...
dst.name = new byte[this.name.length];
    System.arraycopy(this.name, 0, dst.name, 0, this.name.length);dst.speed = this.speed;if(this.bag != null) {
