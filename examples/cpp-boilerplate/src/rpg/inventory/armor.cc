...
namespace rpg {
  ...
  namespace inventory {
    ...
    Armor::Armor(uint64_t defense, std::string name, uint8_t quality, uint64_t cost)
      : defense_(defense), name_(name), quality_(quality), cost_(cost) {}

    void Armor::Init(uint64_t defense, std::string name, uint8_t quality, uint64_t cost) {
      defense_ = defense;
      name_ = name;
      quality_ = quality;
      cost_ = cost;
