...
public:
  ...
  Character(std::string name, rpg::Position position, float speed, rpg::inventory::Inventory *bag,
            std::map<std::string, rpg::inventory::Item *> equipment, std::array<double, 8> buffs);
  ...
  inline const std::map<std::string, rpg::inventory::Item *> &
  GetEquipment() const { return equipment_; }
  inline void SetEquipment(
    const std::map<std::string, rpg::inventory::Item *> &equipment) { equipment_ = equipment; }

  inline rpg::inventory::Item *GetEquipment(const std::string &key) { return equipment_[key]; }
  inline void
  SetEquipment(const std::string &key, rpg::inventory::Item *value) { equipment_[key] = value; }
  ...
  static void WriteDirectTo(bufobjects::BufferBuilder &bb, const std::string &name,
                            const rpg::Position &position, const float &speed,
                            const rpg::inventory::Inventory &bag,
                            const std::map<std::string, rpg::inventory::Item *> &equipment,
