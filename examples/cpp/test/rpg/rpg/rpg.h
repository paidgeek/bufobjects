...
protected:
std::string name_;rpg::Position position_;float speed_;rpg::inventory::Inventory* bag_ = nullptr;std::map<std::string, /* this comment seems to fix a jtwig bug "true" */
    
      rpg::inventory::Item*> equipment_;std::array<double, 8> buffs_;
...
void WriteJsonTo(std::ostream &os);
    ...
    inline void SetEquipment(const std::map<std::string, /* this comment seems to fix a jtwig bug "true" */
    ...
    inline /* this comment seems to fix a jtwig bug "true" */
      ...
      rpg::inventory::Item* GetEquipment(const std::string& key) { return equipment_[key]; }
        inline void SetEquipment(const std::string& key, /* this comment seems to fix a jtwig bug "true" */
