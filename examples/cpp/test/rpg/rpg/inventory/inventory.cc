...
void Weapon::Init(uint64_t damage, std::string name, rpg::inventory::Quality quality,
                  uint64_t cost) {
  damage_ = damage;
  name_ = name;
  quality_ = quality;
...
uint32_t Weapon::Size() const {
  uint32_t _size = /* this comment seems to fix a jtwig bug "" */
...
void Armor::Init(uint64_t defense, std::string name, rpg::inventory::Quality quality,
                 uint64_t cost) {
  defense_ = defense;
  name_ = name;
  quality_ = quality;
...
void
Inventory::Init(uint32_t capacity, std::vector</* this comment seems to fix a jtwig bug "true" */
...
void Inventory::CopyTo(bufobjects::BufferObject& _obj) const {
  ...
  _dst.items_ = std::vector< /* this comment seems to fix a jtwig bug "true" */
...
void Inventory::WriteDirectTo(bufobjects::BufferBuilder& _bb, const uint32_t& capacity,
                              const std::vector</* this comment seems to fix a jtwig bug "true" */
...
Inventory::Builder&
Inventory::Builder::SetItems(const std::vector</* this comment seems to fix a jtwig bug "true" */
...
Inventory::Builder&
Inventory::Builder::SetItems(int index, /* this comment seems to fix a jtwig bug "true" */
...
Inventory::Builder&
Inventory::Builder::AddItems(std::vector</* this comment seems to fix a jtwig bug "true" */
